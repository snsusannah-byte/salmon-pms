...
async def update_invoice(
    ...
    try:
        ...
        invoice_dict = {
            ...
            "parent_invoice_id": updated.parent_invoice_id,
            "notes": updated.notes,
        ...
        if updated.fish_farm_id:
            company_result = await db.execute(
                select(Company.name, Company.code).where(Company.id == updated.fish_farm_id)
            )
            row = company_result.one_or_none()
            if row:
                invoice_dict["fish_farm_name"] = row[0]
                invoice_dict["fish_farm_code"] = row[1]
