...
class ExchangeRecord(Base, TimestampMixin):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    invoice_id: Mapped[int | None] = mapped_column(ForeignKey("import_invoices.id"), nullable=True)
    batch_id: Mapped[int | None] = mapped_column(ForeignKey("batches.id"), nullable=True)
    importer_id: Mapped[int | None] = mapped_column(ForeignKey("companies.id"), nullable=True)  # 进口商
    ...
    notes: Mapped[str | None] = mapped_column(Text)
    related_invoice_ids: Mapped[list | None] = mapped_column(JSON, nullable=True)
    exchange_no: Mapped[str | None] = mapped_column(String(30), nullable=True)
