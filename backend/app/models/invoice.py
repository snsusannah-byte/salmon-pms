...
from app.models.enums import ExchangeStatus, InvoiceStatus
...
class ImportInvoice(Base, TimestampMixin):
    ...
    kill_date: Mapped[Date | None] = mapped_column(Date)
    ...
    fish_farm_id: Mapped[int | None] = mapped_column(ForeignKey("companies.id"))
    ...
    importer_id: Mapped[int] = mapped_column(ForeignKey("companies.id"), nullable=False)  # 进口商
    ...
    # 物流与证书信息
    awb_no: Mapped[str | None] = mapped_column(String(50))  # AWB航空运单号
    gross_weight_kg: Mapped[Decimal | None] = mapped_column(Numeric(12, 3), default=Decimal("0"))  # 毛重(kg)
    eta: Mapped[DateTime | None] = mapped_column(DateTime)  # ETA预计到达
    departure_date: Mapped[Date | None] = mapped_column(Date)  # 发运时间
    flight_info: Mapped[str | None] = mapped_column(String(100))  # 航班信息
    origin_certificate: Mapped[str | None] = mapped_column(String(100))  # 原产地证书
    inspection_certificate: Mapped[str | None] = mapped_column(String(100))  # 检验检疫证书
    
    # 主从发票关系（V8.3新增）
    parent_invoice_id: Mapped[int | None] = mapped_column(ForeignKey("import_invoices.id"), nullable=True)
    is_master: Mapped[bool] = mapped_column(Boolean, default=False)  # 是否主票（AWB级别费用录入方）
    
    # 成本相关（V8.2新增）
    unit_price_usd: Mapped[Decimal | None] = mapped_column(Numeric(12, 4))  # 采购单价(USD)
    estimated_exchange_rate: Mapped[Decimal | None] = mapped_column(Numeric(10, 6))  # 预估汇率
    estimated_cost_cny: Mapped[Decimal | None] = mapped_column(Numeric(15, 2))  # 预估成本(CNY)
    actual_cost_cny: Mapped[Decimal | None] = mapped_column(Numeric(15, 2))  # 实际成本(CNY)
    ...
    importer: Mapped["Company"] = relationship("Company", foreign_keys=[importer_id])
...
class ImportTax(Base, TimestampMixin):
    ...
    consumption_tax: Mapped[Decimal | None] = mapped_column(Numeric(15, 2), default=Decimal("0"))
    other_taxes: Mapped[Decimal | None] = mapped_column(Numeric(15, 2), default=Decimal("0"))
    total_tax: Mapped[Decimal] = mapped_column(Numeric(15, 2), nullable=False)
    bank_account_id: Mapped[int | None] = mapped_column(ForeignKey("bank_accounts.id"), nullable=True)  # 扣款银行（海关税费直接从公账扣除）
    notes: Mapped[str | None] = mapped_column(Text)
...
class ClearanceCost(Base, TimestampMixin):
    ...
    customs_broker_id: Mapped[int | None] = mapped_column(ForeignKey("companies.id"), nullable=True)  # 报关行ID
    customs_broker: Mapped[str | None] = mapped_column(String(100), nullable=True)  # 报关行名称（冗余）
    gross_weight_kg: Mapped[Decimal | None] = mapped_column(Numeric(15, 3), nullable=True)  # 海关出关毛重(kg)
    ...
    inspection_fee: Mapped[Decimal | None] = mapped_column(Numeric(15, 2), default=Decimal("0"))
    quarantine_fee: Mapped[Decimal | None] = mapped_column(Numeric(15, 2), default=Decimal("0"))
    other_costs: Mapped[Decimal | None] = mapped_column(Numeric(15, 2), default=Decimal("0"))
    total_cost: Mapped[Decimal] = mapped_column(Numeric(15, 2), nullable=False)
    payment_type: Mapped[str] = mapped_column(String(20), default="monthly")  # monthly=月结, cash=现付
    notes: Mapped[str | None] = mapped_column(Text)
