...
class Company(Base, TimestampMixin):
    ...
    company_full_name: Mapped[str | None] = mapped_column(String(200))
    brands: Mapped[str | None] = mapped_column(String(500))
    ...
    cooperation_date: Mapped[Date | None] = mapped_column(Date)
    contact_person: Mapped[str | None] = mapped_column(String(100))
    phone: Mapped[str | None] = mapped_column(String(50))
    email: Mapped[str | None] = mapped_column(String(100))
    address: Mapped[str | None] = mapped_column(Text)
    registration_code: Mapped[str | None] = mapped_column(String(100))  # CN海关准入号
    enterprise_registration_no: Mapped[str | None] = mapped_column(String(100))  # 养殖GGN（GlobalGAP Number）
    coc_cert_no: Mapped[str | None] = mapped_column(String(100))  # 监管链COC认证号
    farming_area: Mapped[str | None] = mapped_column(String(100), default="FAO 27")
    website: Mapped[str | None] = mapped_column(String(255))
    bank_name: Mapped[str | None] = mapped_column(String(200))
    bank_account: Mapped[str | None] = mapped_column(String(100))
    payee: Mapped[str | None] = mapped_column(String(200))  # 收款人
    ...
    # 客户等级（用于成品定义V2价格匹配）
    customer_level: Mapped[str | None] = mapped_column(String(20))  # normal / vip / wholesale / bulk
    customer_type: Mapped[str | None] = mapped_column(String(20))  # normal(普通客户), internal_processor(内部加工厂), oem(代工方)
    is_internal: Mapped[bool] = mapped_column(Boolean, default=False)  # 是否内部客户（加工厂/代工方）
    # 客户预付余额（仅 customer 类型有效）
    prepaid_balance: Mapped[Decimal | None] = mapped_column(Numeric(15, 2), default=Decimal("0"))
