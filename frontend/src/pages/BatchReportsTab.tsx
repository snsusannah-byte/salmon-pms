...
function generateBatchReportHTML(detailData: any, lang: "zh" | "en"): string {
  const isEn = lang === 'en';
  const t = {
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="text-xs text-right">总箱数</TableHead>
                <TableHead className="text-xs text-right">总重量(kg)</TableHead>
              ...
              {isLoading ? (
                ...
                filtered.map((item) => (
                  <TableRow key={item.batch_id}>
                    ...
                    <TableCell className="text-xs text-right">{item.total_boxes || 0}</TableCell>
                    <TableCell className="text-xs text-right">{Number(item.total_weight_kg || 0).toLocaleString()}</TableCell>
                    <TableCell className="text-xs text-right">${Number(item.total_purchase_usd || 0).toLocaleString()}</TableCell>
                    <TableCell className="text-xs text-right">{fmt$(item.total_sales_net)}</TableCell>
                    <TableCell className="text-xs text-right">
                      {item.is_exchange_estimated ? (
                        <span className="text-muted-foreground">{fmt$(Number(item.total_exchange_payment || 0) + Number(item.total_exchange_fee || 0))} <span className="text-[10px] text-amber-500">(估)</span></span>
                      ) : (
                    ...
                    <TableCell className={cn("text-xs text-right font-medium", clsProfit(Number(item.cumulative_profit || 0) + Number(item.net_profit || 0)))}>
                      {fmt$(Number(item.cumulative_profit || 0) + Number(item.net_profit || 0))}
                    </TableCell>
                    <TableCell className="text-center">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div className="border rounded-lg p-2.5 space-y-1.5">
                        <p className="text-xs font-semibold text-green-600">
                          {detailLang === "zh" ? "购汇登记" : "Exchange"}
                          {detailData.is_exchange_estimated && (
                            <span className="ml-1 text-[10px] text-amber-500 font-normal">(估算)</span>
                          )}
                        </p>
                        ...
                        {(() => {
                          ...
                          return (
                            <>
                              ...
                              {Number(cb.extra_expenses || 0) > 0 && (() => {
                                const extraItem = detailData.clearance_extra_items?.[0];
                                const desc = extraItem?.description || "";
                                return (
                                  <div className="flex justify-between text-xs">
                                    <span className="text-muted-foreground">
                                      {detailLang === "zh" ? "额外支出" : "Extra Expenses"}
                                      {desc && <span className="text-muted-foreground/60 ml-1">({desc})</span>}
                                    </span>
                                    <span>{fmt$(cb.extra_expenses)}</span>
                                  </div>
                                );
                              })()}
